...
struct ContentView: View {
    var body: some View {
        VStack {
            Text("number of increases (single): \(countIncreases(readings: loadFile(fileName: "input") ?? [] ))")
            Text("number of increases (rolling): \(countIncreases(readings: rollingTotals(readings: loadFile(fileName: "input") ?? [] )))")
        }
    }
    ...
    func countIncreases(readings: [Int]) -> Int {
        var counter = 0
        for i in 0..<readings.count where i > 0 && readings[i] > readings[i-1] { counter += 1 }
        return counter
    }
